...
router.get('/restaurant/:restaurantId', async (req, res) => {
    try {
        ...
        const { status, payment_status, date_from, date_to, table_id, customer_name, limit = '50', page = '1' } = req.query;
...
router.post('/', async (req, res) => {
    try {
        ...
        const order = await prisma.$transaction(async (tx) => {
            ...
            // Create order items
            const orderItems = await Promise.all(validatedData.order_items.map(item => tx.orderItem.create({
                data: {
...
router.get('/restaurant/:restaurantId/stats', async (req, res) => {
    try {
        ...
        const [totalOrders, totalRevenue, pendingOrders, completedOrders, cancelledOrders, averageOrderValue] = await Promise.all([
            prisma.order.count({ where }),
